...
//     try {
//         const { name, email, role, organizationId, createdBy } = req.body;

//         if (!email || !role || !organizationId || !name || !createdBy)
//             return res.status(400).json({ message: "All Fields Are Required" });

//         // if (req.user.createdBy === "user" && req.user.role === "user") {
...
const createUser = async (req, res) => {
    try {
        ...
        // CASE 1: User created by admin → already has organization
        if (creator.organization) {
            finalOrganizationId = creator.organization;
        }
        // CASE 2: Admin → must pick organization from input
        else {
            if (!organizationId) {
